...
**Shake.** The same protocol, window geometry, criterion and interval procedure apply to head shakes, with one difference. No frozen 60 s shake threshold exists, and because the 60 s configuration does not assume that rotation channel 0 is anatomical pitch, the shake axis is not fixed a priori either. It is selected on shake DEV jointly with the amplitude threshold over the three rotation channels, and the full three-axis DEV table is reported. That is one additional disclosed DEV decision relative to nod, which inherited its axis from the frozen configuration. Metric code is shared between the two tasks in `src/windowed_baselines.py` so that the reported numbers cannot diverge through duplicated implementations.

**Files.** `scripts/evaluate_windowed_nod_baselines.py` writes `results/windowed_nod/baselines_bacc/`; `scripts/evaluate_windowed_shake_baselines.py` writes `results/windowed_shake/baselines_bacc/`; the earlier F1-selected run is retained unchanged at `results/windowed_nod/baselines/` for comparison. The frozen 60 s threshold is transferred without refitting from `results/rule_selected_config.json`.
