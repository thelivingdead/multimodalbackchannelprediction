...
7. **VideoMAE / fusion — never run.** `scripts/15_train_videomae.py`, `scripts/16_evaluate_videomae.py`, `scripts/17_train_fusion.py` are documented planned-experiment notes with no implementation; `configs/videomae_frozen.yaml` says `status: planned_not_run`; `figures/videomae/` is empty. No pixel model score exists anywhere. (The earlier `src/videomae_model.py` / `src/fusion_model.py` guard stubs were removed in the 20 Aug cleanup.)
