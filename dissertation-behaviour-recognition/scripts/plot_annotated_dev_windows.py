#!/usr/bin/env python3
"""Clear DEV sliding-window figures from the annotated events.

DEV only. No TEST, no metrics. Does not write an overview collage.
"""
...
from src.windowed_protocol import EVENTS_CSV, WINDOWS_DEV_CSV  # noqa: E402

OUT_DIR = ROOT / "results" / "windowed_dev" / "clip_windows"
EXAMPLE = ROOT / "results" / "windowed_dev" / "gold_001_sliding_windows.png"
OVERVIEW = ROOT / "results" / "windowed_dev" / "annotated_clips_overview.png"
...
def _load() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if (win["split"].astype(str).str.upper() != "DEV").any():
        raise SystemExit("STOP: nod_windows_dev.csv contains a non-DEV row")
    if (win["sample_id"].astype(str).str.extract(r"(\d+)", expand=False).astype(int) >= 16).any():
        raise SystemExit("STOP: TEST sample leaked into nod_windows_dev.csv")
    return ev, win
...
def plot_gold_001(events: pd.DataFrame, windows: pd.DataFrame) -> None:
    ...
    ax_t.axis("off")
    ax_t.set_title(
        "gold_001 — your nods on the 60 s clip",
        loc="left",
        fontsize=18,
        color=INK,
        pad=10,
    ...
    ax_w.set_ylim(0, 8)
    ax_w.set_title(
        "Training slices  ·  3 seconds long  ·  next slice starts 2 seconds later",
        loc="left",
        fontsize=16,
        color=INK,
        pad=8,
...
def plot_each_clip(events: pd.DataFrame, windows: pd.DataFrame) -> None:
    ...
    for i in range(1, 16):
        ...
        for j, (_, r) in enumerate(ev.iterrows(), start=1):
            ...
            ax.text((s + e) / 2, 2.15, f"Nod {j}\n{s:.1f}–{e:.1f} s", ha="center", fontsize=11, color=NOD, fontweight="600")
        ...
        ax.text(0, 3.68, "Each block is one 3 s training slice (step 2 s). Green edge = YES.", fontsize=12, color=MUTED)
...
def main() -> None:
    ...
    print("removed annotated_clips_overview.png")
