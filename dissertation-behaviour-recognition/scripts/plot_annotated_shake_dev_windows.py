#!/usr/bin/env python3
"""DEV shake sliding-window figures. Does not write nod figures."""
...
from src.windowed_protocol import SHAKE_EVENTS_CSV, SHAKE_WINDOWS_DEV_CSV  # noqa: E402

OUT_DIR = ROOT / "results" / "windowed_shake" / "clip_windows"
EXAMPLE = ROOT / "results" / "windowed_shake" / "gold_001_sliding_windows.png"
...
def _load() -> tuple[pd.DataFrame, pd.DataFrame]:
    ev = pd.read_csv(SHAKE_EVENTS_CSV)
    win = pd.read_csv(SHAKE_WINDOWS_DEV_CSV)
    if (win["split"].astype(str).str.upper() != "DEV").any():
        raise SystemExit("STOP: shake_windows_dev.csv contains a non-DEV row")
    return ev, win


def plot_gold_001(events: pd.DataFrame, windows: pd.DataFrame) -> None:
    sid = "gold_001"
    ...
    ax_t.axis("off")
    ax_t.set_title(
        "gold_001 — your shakes on the 60 s clip",
        loc="left",
        fontsize=18,
        color=INK,
        pad=10,
...
def main() -> None:
    ...
    print(f"wrote {OUT_DIR.relative_to(ROOT)}/gold_001.png … gold_015.png")
