#!/usr/bin/env python3
"""TEST shake sliding-window figures. Does not write nod or DEV shake figures."""
...
from src.windowed_protocol import SHAKE_EVENTS_TEST_CSV, SHAKE_WINDOWS_TEST_CSV  # noqa: E402

OUT_DIR = ROOT / "results" / "windowed_shake_test" / "clip_windows"
EXAMPLE = ROOT / "results" / "windowed_shake_test" / "gold_016_sliding_windows.png"
...
def _load() -> tuple[pd.DataFrame, pd.DataFrame]:
    ev = pd.read_csv(SHAKE_EVENTS_TEST_CSV) if SHAKE_EVENTS_TEST_CSV.exists() else pd.DataFrame()
    win = pd.read_csv(SHAKE_WINDOWS_TEST_CSV)
    if (win["split"].astype(str).str.upper() != "TEST").any():
        raise SystemExit("STOP: shake_windows_test.csv contains a non-TEST row")
    nums = win["sample_id"].astype(str).str.extract(r"(\d+)", expand=False).astype(int)
    if ((nums < 16) | (nums > 30)).any():
        raise SystemExit("STOP: non-TEST sample leaked into shake_windows_test.csv")
    return ev, win


def plot_gold_016(events: pd.DataFrame, windows: pd.DataFrame) -> None:
    sid = "gold_016"
    ev = events.loc[events["sample_id"] == sid].sort_values("start_sec") if not events.empty else events
    ...
    ax_t.axis("off")
    ax_t.set_title(
        "gold_016 — your shakes on the 60 s clip",
        loc="left",
        fontsize=18,
        color=INK,
        pad=10,
    ...
    ax_t.text(
        0,
        2.85,
        f"{len(ev)} shakes marked   ·   {n_pos} of {len(ww)} three-second windows contain a shake",
        fontsize=13,
        color=MUTED,
    )
    ...
    for i, (_, r) in enumerate(ev.iterrows(), start=1):
        ...
        ax_t.axvspan(s, max(e, s + 0.35), ymin=0.38, ymax=0.62, color=SHAKE, alpha=0.85)
        ax_t.annotate(
            f"Shake {i}\n{s:.1f}–{e:.1f} s",
            xy=((s + e) / 2, 1.42),
            xytext=((s + e) / 2, 2.25),
            ...
            color=SHAKE,
            fontweight="600",
            arrowprops=dict(arrowstyle="-", color=SHAKE, lw=1.0),
        )
    ax_w.axis("off")
    ax_w.set_xlim(0, 10)
    ax_w.set_ylim(0, 8)
    ...
    ax_w.text(0, 7.45, "YES means that 3 s slice overlaps one of the shakes above.", fontsize=13, color=MUTED)
    cols = 5
    ...
    for idx, r in enumerate(ww.itertuples(index=False)):
        ...
        ax_w.text(x + box_w / 2, y + 0.58, f"{r.start_sec:g}–{r.end_sec:g} s", ha="center", va="center", fontsize=13, color=INK, fontweight="600")
        ax_w.text(
            x + box_w / 2,
            y + 0.24,
            "YES  shake" if yes else "no shake",
            ha="center",
            va="center",
            fontsize=12,
...
def plot_each_clip(events: pd.DataFrame, windows: pd.DataFrame) -> None:
    ...
    for i in range(16, 31):
        ...
        ax.axis("off")
        ax.set_title(
            f"{sid}   ·   {len(ev)} shake{'s' if len(ev) != 1 else ''}   ·   "
            f"{n_pos} of {len(ww)} windows contain a shake",
            loc="left",
            fontsize=16,
            color=INK,
            pad=8,
        ...
        for j, (_, r) in enumerate(ev.iterrows(), start=1):
            s, e = float(r.start_sec), float(r.end_sec)
            ax.axvspan(s, max(e, s + 0.35), ymin=0.36, ymax=0.52, color=SHAKE, alpha=0.9)
            y_lab = 2.35 if j % 2 == 0 else 2.05
            ax.text((s + e) / 2, y_lab, f"Shake {j}\n{s:.1f}–{e:.1f} s", ha="center", fontsize=11, color=SHAKE, fontweight="600")
        ...
        ax.text(0, 3.68, "Each block is one 3 s window (step 2 s). Green edge = YES.", fontsize=12, color=MUTED)
